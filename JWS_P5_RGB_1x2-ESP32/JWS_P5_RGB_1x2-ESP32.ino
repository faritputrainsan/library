/*************************************************************************************
  Waktu Sholat P5 HUB75   - Program jam petunjuk waktu Sholat otomatis yang dilekapi dengan
  ...
  8. Program dipecah menjadi 7 File sehingga maintenance bisa lebih mudah dan aman dan tidak mengganggu bagian
    lain.
  ...
  Semoga Bermanfaat
  Salam Mujahid 212.
 edit by: Dhini Naila tonymrblack@gmail.com
**************************************************************************************/
...
VirtualMatrixPanel virtualDisp(matrix, NUM_ROWS, NUM_COLS, PANEL_RES_X, PANEL_RES_Y, false);
...
//  set_default_prm();
  Serial.begin(115200);
  SerialBT.begin("RUNNINGTEKSSMD"); //Bluetooth device name
...
void loop()
{
  ...
  // =========================================
  if (DoSwap) {
    matrix.flipDMABuffer(); // Swap Buffer if Change
  }
}
// =========================================
